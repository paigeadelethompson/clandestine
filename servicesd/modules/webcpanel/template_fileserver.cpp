...
struct ForLoop {
    ...
    typedef std::pair<TemplateFileServer::Replacements::iterator, TemplateFileServer::Replacements::iterator>
    range;
    std::vector<range> ranges; /* iterator ranges for each variable */

    ForLoop(size_t s, TemplateFileServer::Replacements &r,
            const std::vector<Anope::string> &v,
            const std::vector<Anope::string> &r_names) : start(s), vars(v) {
...
static Anope::string FindReplacement(const TemplateFileServer::Replacements &r,
...
TemplateFileServer::TemplateFileServer(const Anope::string &f_n) : file_name(
        f_n) {
}

void TemplateFileServer::Serve(HTTPProvider *server,
                               const Anope::string &page_name, HTTPClient *client, HTTPMessage &message,
                               HTTPReply &reply, Replacements &r) {
    int fd = open((template_base + "/" + this->file_name).c_str(), O_RDONLY);
    if (fd < 0) {
        Log(LOG_NORMAL, "httpd") << "Error serving file " << page_name << " (" <<
                                 (template_base + "/" + this->file_name) << "): " << strerror(errno);
    ...
    for (unsigned j = 0; j < buf.length(); ++j) {
        if (buf[j] == '\\' && j + 1 < buf.length() && (buf[j + 1] == '{'
                || buf[j + 1] == '}')) {
            escaped = true;
            ...
            if (content.find("IF ") == 0) {
                ...
                if (tokens.size() == 4 && tokens[1] == "EQ") {
                    Anope::string first = FindReplacement(r, tokens[2]), second = FindReplacement(r,
                                          tokens[3]);
                    if (first.empty()) {
                        first = tokens[2];
                    }
                ...
                if (tokens.size() != 2) {
                    ...
                    if (!finished.empty()) {
                        reply.Write(
                            finished); // Write out what we have currently so we insert this files contents here
                        finished.clear();
                    }
