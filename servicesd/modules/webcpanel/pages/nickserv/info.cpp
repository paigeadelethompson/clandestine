...
WebCPanel::NickServ::Info::Info(const Anope::string &cat,
                                const Anope::string &u) : WebPanelProtectedPage(cat, u) {
}

bool WebCPanel::NickServ::Info::OnRequest(HTTPProvider *server,
    ...
    if (message.post_data.empty() == false) {
        if (message.post_data.count("email") > 0) {
            if (message.post_data["email"] != na->nc->email) {
                if (!message.post_data["email"].empty()
                        && !Mail::Validate(message.post_data["email"])) {
        ...
        if (message.post_data.count("greet") > 0) {
            ...
            const Anope::string &post_greet = HTTPUtils::URLDecode(
                                                  message.post_data["greet"].replace_all_cs("+", " "));
        ...
        } else if (message.post_data["kill"] == "quick"
                   && !na->nc->HasExt("KILL_QUICK")) {
            na->nc->Extend<bool>("KILLPROTECT");
            na->nc->Extend<bool>("KILL_QUICK");
            replacements["MESSAGES"] = "Kill updated";
        } else if (message.post_data["kill"] == "off" && (na->nc->HasExt("KILLPROTECT")
                   || na->nc->HasExt("KILL_QUICK"))) {
