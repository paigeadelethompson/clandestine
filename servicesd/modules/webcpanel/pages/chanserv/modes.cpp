...
WebCPanel::ChanServ::Modes::Modes(const Anope::string &cat,
                                  const Anope::string &u) : WebPanelProtectedPage(cat, u) {
}

bool WebCPanel::ChanServ::Modes::OnRequest(HTTPProvider *server,
    ...
    bool has_priv = na->nc->IsServicesOper()
                    && na->nc->o->ot->HasPriv("chanserv/administration");
    ...
    if (cm) {
        if (message.get_data["del"].empty() == false
            ...
            WebPanel::RunCommand(client, na->nc->display, na->nc, "ChanServ",
                                 "chanserv/mode", params, replacements);
