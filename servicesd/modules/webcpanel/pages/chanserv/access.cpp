...
WebCPanel::ChanServ::Access::Access(const Anope::string &cat,
                                    const Anope::string &u) : WebPanelProtectedPage(cat, u) {
}

bool WebCPanel::ChanServ::Access::OnRequest(HTTPProvider *server,
    ...
    bool has_priv = na->nc->IsServicesOper()
                    && na->nc->o->ot->HasPriv("chanserv/access/modify");
    ...
    if (u_access.HasPriv("ACCESS_CHANGE") || has_priv) {
        if (message.get_data["del"].empty() == false
                && message.get_data["mask"].empty() == false) {
            std::vector<Anope::string> params;
            params.push_back(ci->name);
            params.push_back("DEL");
            params.push_back(message.get_data["mask"]);

            WebPanel::RunCommand(client, na->nc->display, na->nc, "ChanServ",
                                 "chanserv/access", params, replacements);
        } else if (message.post_data["mask"].empty() == false
                   && message.post_data["access"].empty() == false
                   && message.post_data["provider"].empty() == false) {
            ...
            if (provider == "chanserv/access") {
                ...
                WebPanel::RunCommandWithName(client, na->nc, "ChanServ", "chanserv/xop",
                                             message.post_data["access"], params, replacements);
                ...
                WebPanel::RunCommand(client, na->nc->display, na->nc, "ChanServ",
                                     "chanserv/flags", params, replacements);
            }
        }
    }
    ...
    replacements["ACCESS_CHANGE"] = u_access.HasPriv("ACCESS_CHANGE") ? "YES" :
                                    "NO";
