...
WebCPanel::OperServ::Akill::Akill(const Anope::string &cat,
                                  const Anope::string &u) : WebPanelProtectedPage(cat, u) {
}

bool WebCPanel::OperServ::Akill::OnRequest(HTTPProvider *server,
    ...
    if (!na->nc->o || !na->nc->o->ot->HasCommand("operserv/akill")) {
        ...
        if (message.post_data.count("mask") > 0
                && message.post_data.count("expiry") > 0
                && message.post_data.count("reason") > 0) {
            ...
            WebPanel::RunCommand(client, na->nc->display, na->nc, "OperServ",
                                 "operserv/akill", params, replacements);
        }
